#!/usr/bin/env python3
"""
Pipeline script to regenerate a training JSONL file using simulated eye-tracking scan-paths.

Steps:
1. Stream through `completion_train_final.jsonl` to extract the *code* for each
   example (we ignore existing eye-tracking metadata).
2. For each code snippet, call helper utilities from `sti_codesearchnet.py` to
   simulate a scan-path (sequence of fixated line numbers).
3. Convert the simulated scan-path into the token-level structure expected by
   our advanced SFT scripts – closely mirroring the logic in
   `scanpath_to_completion_jsonl.py`.
4. Write a new JSONL file where every record has **fresh**
   `mask`, `line_numbers`, `semantic_token_sequence`, `ngram_indices`, and
   `order_sequence` derived from the simulated scan-path.

NOTE:
•  The script purposefully *does not* reproduce the full `code_tokens` parsing
   of the original dataset (which required a complex tokenizer).  Instead, it
   tokenises at the **line** level (one token per line) like the original
   scan-path conversion script.  This is sufficient for experiments that focus
   on line-level fixation.
•  If you need word-level tokens, plug in your tokenizer where indicated.
"""
# ---------------------- imports ----------------------
import json
import os
...
# Keyword set for fallback semantic mapping
JAVA_KEYWORDS_SET = set(
...
def extract_java_ast_semantic_types(code: str) -> Dict[int, str]:
    ...
    try:
        ...
        # Map line number to (node, type)
        line_to_type = {}
        
        # Collect all AST nodes with their positions
        for path, node in tree:
            # Only consider nodes with position (i.e., real code lines)
            if hasattr(node, 'position') and node.position:
                line = node.position[0]
                stmt_type = type(node).__name__
#                print(stmt_type)
                # Keep the most specific node type per line
                if line not in line_to_type:
                    line_to_type[line] = (node, stmt_type)
 #                   print(line_to_type)
                else:
                    # Prefer more specific statement types
                    current_type = line_to_type[line][1]
                    if stmt_type in ['IfStatement', 'ForStatement', 'WhileStatement', 'TryStatement', 
                                   'CatchClause', 'ReturnStatement', 'Assignment', 'MethodInvocation',
                                   'VariableDeclarator', 'ClassDeclaration', 'MethodDeclaration']:
                        line_to_type[line] = (node, stmt_type)
        
        # Map line numbers to semantic types
        for line_num, (node, stmt_type) in line_to_type.items():
            line_to_semantic[line_num] = stmt_type
            
    except Exception as e:
        # AST parsing failed, return empty dict to trigger fallback
        pass
...
# Use the Java-specific scan-path simulator
try:
    import sti_codesearchnet_java as sti  # noqa: F401  (aliased as generic 'sti')
...
# Config
# ------------------------------------------------------------
ORIG_FILE = os.path.join(SCRIPT_DIR, "completion_train_final.jsonl")
OUT_FILE = os.path.join(SCRIPT_DIR, "completion_train_final_scanpath.jsonl")
MAX_SAMPLES = None  # keep for quick debugging; overridden by CLI range if provided
NUM_SIMULATIONS = 1
VERBOSE_EVERY = 50

# Path to semantic-index table used in scanpath_to_completion_jsonl.py
...
def main():
    parser = argparse.ArgumentParser(description="Generate scan-path augmented training JSONL (Java)")
    ...
    parser.add_argument("--model_id", type=str, default="meta-llama/Llama-3.2-1B", help="HF model id for tokenizer (not used in line-level mode)")
    parser.add_argument("--auth_token", type=str, default=None, help="HF auth token if required (not used in line-level mode)")
    ...
    with open(ORIG_FILE, "r", encoding="utf-8") as fin, open(OUT_FILE, "w", encoding="utf-8") as fout:
        for line_idx, line in enumerate(fin):
            ...
            if line_idx < start_idx:
                continue
            # Stop at end_idx if provided and not None
