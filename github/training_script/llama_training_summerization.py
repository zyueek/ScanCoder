...
TRAIN_FILE = os.path.join(DATA_FOLDER, f"{TASK}_train_final_scanpath.jsonl")
VALID_FILE = os.path.join(DATA_FOLDER, f"{TASK}_valid_final_scanpath.jsonl")
MAX_LENGTH = 1024

# --- Seed Configuration ---
RANDOM_SEED = 42  # Default seed for reproducible training
...
def create_and_process_data(tokenizer):
 ...
 #           print(f"  {i}: {token_id} -> '{token}' | {label_str} | weight:{weight}")
    ...
    candidate_remove = [
        ...
        'semantic_token_sequence', 'line_numbers', 'mask', 'ngram_indices', 'order_sequence'
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Llama model for code summarization with scanpath augmentation")
    parser.add_argument("--seed", type=int, default=RANDOM_SEED, help="Random seed for reproducible training")
    ...
    # Ensure output directory exists and check disk space
    try:
        ...
        if free_gb < 5:  # Require at least 5GB free space
            logging.error(f"Insufficient disk space. Only {free_gb} GB available. Need at least 5 GB.")
            sys.exit(1)
    ...
    training_args = TrainingArguments(
        ...
        logging_steps=10, 
        eval_strategy="epoch",
        # Disable checkpoint saving during training to avoid disk space issues
        save_strategy="no",
        save_total_limit=None,
        load_best_model_at_end=False,
        dataloader_num_workers=0,
        seed=seed,  # Set seed for training reproducibility
        # Add safety measures for saving
        save_safetensors=False,  # Use pickle format which is more stable
 ...
 #   print(tokenized_datasets["train"][0])
    ...
    try:
        ...
        if "out of memory" in str(e).lower() or "cuda" in str(e).lower():
            ...
            logging.info("Attempting to reduce batch size and retry...")
            
            # Reduce batch size and retry
            training_args.per_device_train_batch_size = 1
            training_args.gradient_accumulation_steps = 16  # Keep effective batch size similar
    ...
    try:
        ...
        logging.info(f"Final model saved successfully to {OUTPUT_DIR}")
    except Exception as e:
        logging.error(f"Error saving model: {e}")
        logging.info("Attempting to save model with reduced precision...")
        try:
            # Try saving with reduced precision to save space
            trainer.save_model(safe_serialization=False)
            tokenizer.save_pretrained(OUTPUT_DIR)
            logging.info(f"Model saved with reduced precision to {OUTPUT_DIR}")
        except Exception as e2:
            logging.error(f"Failed to save model even with reduced precision: {e2}")
            logging.error("Training completed but model could not be saved. Check disk space.")
